    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          <a href="/" aria-label="Ir al inicio" onClick={(event) => { event.preventDefault(); handleNavigation(''); }} className="flex-shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70 focus-visible:ring-offset-2 focus-visible:ring-offset-white rounded-lg">
                ...
                <div role="menu" className="absolute left-1/2 top-full mt-2 grid w-[560px] -translate-x-1/2 grid-cols-2 gap-x-8 gap-y-3 rounded-2xl border border-white/10 bg-[#0D0D0D] p-6 shadow-2xl">
                  ...
                  {sitemapDestinations.map((destination) => (
                    <a
                      ...
                      className={`flex min-h-11 items-center gap-2 rounded-lg px-3 py-2 text-sm leading-snug transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70 ${activePage === destination.slug ? 'font-semibold text-amber-500 bg-white/5' : 'text-slate-400 hover:text-white'}`}
                    >
          ...
          <>
            ...
            >
              <div className="flex min-h-12 items-center justify-between border-b border-white/10 pb-3">
                <a href="/" aria-label="Ir al inicio" onClick={(event) => { event.preventDefault(); handleNavigation(''); }} className="flex-shrink-0 rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70 focus-visible:ring-offset-2 focus-visible:ring-offset-white">
                  <img src="/img/brand-dark.png" alt="Mudanzas Miranda" className="block h-[38px] w-auto object-contain sm:h-[45.6px]" />
                </a>
                <button
                  type="button"
              ...
              <nav className="mt-3 flex flex-col gap-1" aria-label="Navegación móvil">
                <a href="/" onClick={(event) => { event.preventDefault(); handleNavigation(''); }} className={`flex min-h-12 items-center gap-3 rounded-xl px-4 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70 ${activePage === '' ? 'text-amber-500 bg-white/5' : 'text-slate-300'}`}>
                  <Home className="h-4 w-4" /> Inicio
                </a>
                <a href="/nosotros.html" onClick={(event) => { event.preventDefault(); handleNavigation('nosotros'); }} className={`flex min-h-12 items-center gap-3 rounded-xl px-4 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70 ${activePage === 'nosotros' ? 'text-amber-500 bg-white/5' : 'text-slate-300'}`}>
                  <Info className="h-4 w-4" /> Nosotros
                </a>

                <div className="mt-2 border-t border-white/10 pt-2">
                  <button type="button" aria-expanded={openMobileMenu === 'services'} onClick={() => setOpenMobileMenu(openMobileMenu === 'services' ? null : 'services')} className="flex min-h-12 w-full items-center justify-between rounded-xl px-4 text-sm font-semibold text-slate-300 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70">
